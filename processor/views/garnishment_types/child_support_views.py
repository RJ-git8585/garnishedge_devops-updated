...
from processor.models import WithholdingRules, WithholdingLimit, DeductionPriority, MultipleGarnPriorityOrders, State, GarnishmentType
from processor.serializers.shared_serializers import MultipleGarnPriorityOrderCRUDSerializer
...
class MultipleGarnPriorityOrderReorderAPIView(APIView):
    """
    Move a MultipleGarnPriorityOrders record to a new priority and reindex the sequence for given state and garnishment type.
    Request body: { "id": <int>, "state": <name or code>, "garnishment": <type name>, "new_priority": <int> }
    """
    ...
    @swagger_auto_schema(
        request_body=openapi.Schema(
            ...
            properties={
                'id': openapi.Schema(type=openapi.TYPE_INTEGER, description='Record ID to move'),
                'state': openapi.Schema(type=openapi.TYPE_STRING, description='State name or code'),
                'garnishment': openapi.Schema(type=openapi.TYPE_STRING, description='Garnishment type name'),
                'new_priority': openapi.Schema(type=openapi.TYPE_INTEGER, description='1-based new position'),
            },
            required=['id', 'state', 'garnishment', 'new_priority']
        ),
        responses={200: 'Reordered successfully'}
    )
    def post(self, request):
        try:
            record_id = request.data.get('id')
            state_in = request.data.get('state') or request.data.get('state_name') or request.data.get('state_code')
            garn_in = request.data.get('garnishment') or request.data.get('garnishment_type') or request.data.get('type')
            try:
                new_priority = int(request.data.get('new_priority'))
            except Exception:
                return ResponseHelper.error_response("new_priority must be an integer", status_code=status.HTTP_400_BAD_REQUEST)

            if not record_id or not state_in or not garn_in:
                return ResponseHelper.error_response("id, state and garnishment are required", status_code=status.HTTP_400_BAD_REQUEST)

            # Resolve state and garnishment
            state_obj = State.objects.filter(state__iexact=state_in).first() or \
                        State.objects.filter(state_code__iexact=state_in).first()
            if not state_obj:
                return ResponseHelper.error_response(f"State '{state_in}' not found", status_code=status.HTTP_400_BAD_REQUEST)

            gt_obj = GarnishmentType.objects.filter(type__iexact=garn_in).first()
            if not gt_obj:
                return ResponseHelper.error_response(f"GarnishmentType '{garn_in}' not found", status_code=status.HTTP_400_BAD_REQUEST)

            with transaction.atomic():
                rec = MultipleGarnPriorityOrders.objects.select_for_update().get(id=record_id)
                # Scope: all priorities for this state + garnishment type
                items = list(MultipleGarnPriorityOrders.objects.select_for_update().filter(state=state_obj, garnishment_type=gt_obj).order_by('priority_order', 'id'))

                if rec.state_id != state_obj.id or rec.garnishment_type_id != gt_obj.id:
                    return ResponseHelper.error_response("Record does not belong to provided state/garnishment", status_code=status.HTTP_400_BAD_REQUEST)
            ...
            # Return updated order for the scope
            updated = MultipleGarnPriorityOrders.objects.filter(state=state_obj, garnishment_type=gt_obj).order_by('priority_order', 'id').values('id', 'priority_order')
            return ResponseHelper.success_response(message="Reordered successfully", data={'order': list(updated)})
        except MultipleGarnPriorityOrders.DoesNotExist:
            return ResponseHelper.error_response("MultipleGarnPriorityOrders not found", status_code=status.HTTP_404_NOT_FOUND)
        except Exception as e:
